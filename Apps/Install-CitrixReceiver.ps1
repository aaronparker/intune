...
If (!($Receiver) -or ($Receiver.Version -lt $BaselineVersion)) {
    ...
    If (Test-Path $Target) { Write-Verbose -Message "Deleting $Target"; Remove-Item -Path $Target -Force -ErrorAction SilentlyContinue -Verbose }
    ...
    If (Test-Path $Target) {
        ...
        Write-Verbose -Message "Deleting $Target"; Remove-Item -Path $Target -Force -ErrorAction SilentlyContinue -Verbose
        Write-Verbose -Message "Querying for installed Receiver version."
        $Receiver = Get-WmiObject -Class Win32_Product | Where-Object { $_.Name -Like "Citrix Receiver Inside*" } | Select-Object Name, Version
        Write-Verbose -Message "Installed Receiver: $($Receiver.Version)."
