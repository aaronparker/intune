<#
    .SYNOPSIS
        Get legacy Microsoft Store for Business Apps from an Intune tenant
...
process {
    $params = @{
        Uri         = "https://graph.microsoft.com/beta/deviceAppManagement/mobileApps?`$filter=(isof('microsoft.graph.microsoftStoreForBusinessApp'))"
        Method      = "GET"
        Headers     = $authHeader
        ErrorAction = "Stop"
    }
